...
module.exports = async ({ github, context }) => {
    ...
    for (const label of context.payload.issue.labels) {
        if (label.name.includes(CONSTANT_VALUES.GLOBALS.LABELS.BUG) ||
            ...
            const params_1 = new URLSearchParams({ ${CONSTANT_VALUES.MODULE.CSAT.SATISFACTION_PARAM}.toString(): CONSTANT_VALUES.MODULE.CSAT.YES, ${CONSTANT_VALUES.MODULE.CSAT.ISSUEID_PARAM}.toString(): issue });
            const link_1 = `${baseUrl}?${params_1.toString()}`;
            const params_2 = new URLSearchParams({ ${CONSTANT_VALUES.MODULE.CSAT.SATISFACTION_PARAM}.toString(): CONSTANT_VALUES.MODULE.CSAT.NO, ${CONSTANT_VALUES.MODULE.CSAT.ISSUEID_PARAM}.toString(): issue });
